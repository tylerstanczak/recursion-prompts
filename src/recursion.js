...
var sumBelow = function(n) {
  if (n < 2 && n > -2) return 0;
  var negativity = (n < 0);
  var number = (negativity) ? (n+1) : (n-1);
  return (negativity) ? (number + sumBelow(n+1)) : number + sumBelow(n-1);
};
...
var palindrome = function(string) {
  ...
  return (string[0] === string[string.length-1]) ? palindrome(string.slice(1, string.length-1)) : false;
};
...
var createArray = function(str) {
  return (str[0]) ? [str[0]].concat(createArray(str.slice(1))) : [];
};

// 17. Reverse the order of an array
var reverseArr = function(array) {
  return (array[array.length-1]) ? [array[array.length-1]].concat(reverseArr(array.slice(0, array.length-1))) : [];
};
...
var fizzBuzz = function(n) {
  var nfb;
  if ((modulo(n, 3) === 0) && (modulo(n, 5) === 0)) nfb = 'FizzBuzz';
  else if (modulo(n, 3) === 0) nfb = 'Fizz';
  else if (modulo(n, 5) === 0) nfb = 'Buzz';
  else nfb = n.toString();
  return (n === 1) ? ['1'] : fizzBuzz(n-1).concat([nfb]);
};
...
var countKeysInObj = function(obj, key) {
  ...
  for (var k in keyList) {
    if (obj[keyList[k]] instanceof Object) count+= countKeysInObj(obj[keyList[k]], key);
    if (keyList[k] === key) count++;
  }
  return count;
};
